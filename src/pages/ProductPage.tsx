...
export default function ProductPage() {
  ...
  const [fetchProduct, isLoading, error] = useFetching(
    async (productId: string) => {
      const data = await ProductService.getProductById(productId);
      setProduct(data);
    },
  );
  const [fetchProductComments, isLoadingComments, errorComments] = useFetching(
    async (productId: string) => {
      ...
      <div className="d-flex flex-column gap-3">
        {productComments.map((comment, index) => (
          <div key={index} className="p-2 bg-dark text-white">
            <p>
              {comment.user.username} - {("0" + dateObj.getUTCDay()).slice(-2)}.
              {("0" + dateObj.getUTCMonth()).slice(-2)}.{dateObj.getFullYear()}
            </p>
            <p>{comment.body}</p>
          </div>
        ))}
